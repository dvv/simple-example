...
module.exports = testCase
	...
	testAdmin: (test) ->
		...
		Next context,
			...
			(err, result, next) ->
				test.ok not err, 'added ok'
				test.ok result.id is 'x' and not result.name, 'added ok'
				@[@entity].add.call @, {id: 'a', name: 'Foo2'}, next
			(err, result, next) ->
				test.equal err, 'Duplicated', 'duplicated nak'
				@[@entity].add.call @, {id: 'y', name: 'Foo2'}, next
			(err, result, next) ->
				test.ok not err and result.id is 'y' and not result.name, 'added ok'
				@[@entity].add.call @, {id: 'z', name: 'Foo3'}, next
			(err, result, next) ->
				test.ok not err and result.id is 'z' and not result.name, 'added ok'
