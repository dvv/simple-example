...
module.exports = testCase({
  setUp: function(callback) {
    return app.getContext('z', function(err, result) {
      context = result;
      return callback();
    });
  },
  ...
  testAdmin: function(test) {
    ...
    return Next(context, function(err, result, next) {
      ...
      test.ok(!err, 'query ok');
      test.ok(result.length === 1 && result[0].id === 'z', 'only self record (id = z) should survive');
      return this[this.entity].add.call(this, {
        id: 'x',
        name: 'Foo1'
      }, next);
    }, function(err, result, next) {
      test.ok(!err, 'added ok1');
      test.ok(result.id === 'x' && !result.name, 'added ok2');
      return this[this.entity].add.call(this, {
        id: 'z',
        name: 'Foo2'
      }, next);
      ...
      test.ok(!err && result.id === 'y' && !result.name, 'added ok3');
  ...
  testLanguage: function(test) {
    ...
    return Next(context, function(err, result, next) {
      ...
      test.ok(!err, 'added ok');
      test.ok(result.id === 'en' && result.name === 'English', 'added ok');
      return this[this.entity].add.call(this, {
        id: 'ru',
        name: 'Russian',
        localName: 'Русский'
      ...
      test.ok(!err, 'added ok');
      return this[this.entity].query.call(this, 'values(localName)', next);
    }, function(err, result, next) {
      test.ok(!err && result.length === 2, 'queried 2 documents');
